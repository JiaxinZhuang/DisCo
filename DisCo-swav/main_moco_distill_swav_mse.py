...
parser.add_argument('--nmb_prototypes', default=0, type=int,
                    help='num prototype')
parser.add_argument('--only-mse', default='store_true',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    elif args.teacher_arch == 'resnet50':
    ...
    model = moco.builder_kq_mse_largeembedding_2048_small_patch.MoCo(
        model,
        teacher_model,
    ...
    train_dataset = Small_Patch_TSVDataset(traindir, augmentation,
                                           small_patch_augmentation,
                                           num_patches=6)
...
def train(train_loader, model, criterion, criterion_mse, optimizer, epoch, args):
    ...
    mse_losses_small_img = AverageMeter('MSE Loss Small', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, mse_losses, mse_losses_qk, mse_losses_small_img, top1, top5],
    ...
    for i, (images) in enumerate(train_loader):
        ...
        # compute output
#TODO
        output, target, student_q, teacher_q, student_qk, teacher_qk,\
            teacher_small, student_small = model(im_q=images[0], im_k=images[1], small_img=images[2])

        loss = criterion(output, target)
        mse_loss = criterion_mse(teacher_q, student_q)
        mse_loss_qk = criterion_mse(teacher_qk, student_qk)
        mse_loss_small_img = criterion_mse(teacher_small, student_small)

        if args.only_mse:
            loss = 0.0 * loss + 1.0 * mse_loss + 1.0 * mse_loss_qk + (1.0/6) *mse_loss_small_img
        else:
            loss = 1.0 * loss + 1.0 * mse_loss + 1.0 * mse_loss_qk + (1.0/6) *mse_loss_small_img
        #loss += mse_loss
        #loss += mse_loss_qk
        #loss += mse_loss_q1
        ...
        mse_losses_small_img.update(mse_loss_small_img.item(), images[0].size(0))
